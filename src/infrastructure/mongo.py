...
class MongoDBService(Generic[T]):
    ...
    def __init__(
        ...
        mongodb_uri: str = settings.DATABASE_HOST,
    ) -> None:
        ...
        try:
            self.client = MongoClient(mongodb_uri, appname="researchpal")
    ...
    def update_documents(self, docs_filter_update_kw: list[tuple] | tuple) -> None:

        if type(docs_filter_update_kw) is tuple:
